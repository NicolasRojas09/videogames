import { filterGames, getGames, loadingGames, opcionChange, orderGames, pagesGames } from "../../redux/actions"
...
const Home = () => {
    ...
    const { games, currentPage, totalJuegos, opcion, loading } = useSelector(state => state)
    const games_per_page = 15

    useEffect(()=>{
        games?.length ? dispatch(loadingGames(false)) : dispatch(loadingGames(true))
        games?.length === 0 && dispatch(getGames()).then(() => dispatch(loadingGames(false)))
    }, [])
    ...
    return (
        <div className={style.contenedorCompleto}>
            {loading 
            ?   <div className={style.contenedorLoading}>
                    <div className={style.loadingSpiner}></div>
                    <h1 className={style.loadingH1}>Loading</h1>
                </div> 
            :
